...
- **Composite ranking weights.** `/api/compare` calculates the final score from explicit user needs (75%), overall POI coverage and geographic evidence (15%), and SAL/LGA profile evidence (10%). User-selected preference weights are applied inside the 75% component. The profile scorer retains every source column from the 75-field SAL and 125-field LGA tables, assigns numeric fields to seven dimensions (employment/industry 25%, income/economic 20%, demographic 15%, socioeconomic 15%, community/culture 10%, regional capacity 10%, data quality 5%), and combines available SAL/LGA scores at 60%/40%. Counts are peer-normalized, age/ancestry/birthplace/language vectors use diversity/evenness measures, and missing values are unavailable rather than zero. The response includes dimension scores and field-usage evidence so the design can be audited.
