...
## 15. Verified Hugging Face Endpoint Runtime

On 17 September 2026, the G2 `q4f16` package was deployed as the private Hugging Face Endpoint `serinea-qwen3-06b`. This is a dedicated endpoint, not the general Hugging Face Inference Provider shown on the model page.

- The custom `handler.py` in the private model repository loads `onnx/model_q4f16.onnx` with ONNX Runtime on a CPU instance.
- `POST /api/ai/review` calls the endpoint with a server-only token, validates its `preferences` and `unsupported` JSON response, and caches a valid result in RDS.
