...
function optionalNumber(value: string | undefined): number | null {
  ...
  if (!Number.isFinite(parsed)) throw new Error(`Invalid numeric subsidy value: ${value}`);
  return parsed;
}
...
export class IncentiveService {
  ...
  private load(): Promise<SubsidyRecord[]> {
    if (!this.recordsPromise) this.recordsPromise = this.readRecords();
    return this.recordsPromise;
  }

  private async readRecords(): Promise<SubsidyRecord[]> {
    ...
    return rows.map((row, index) => {
      ...
      return {
        ...
        locality: normalizeArea(requiredText(row, "locality", line)),
        lgaName: normalizeArea(requiredText(row, "lga_name", line)),
